...
class LossFunction(object):
    ...
    def __call__(self, est, lbl, loss_mask, n_out_channels, n_frames):
        ...
        est_real = torch.cat(
            [est[:, i :, :] for i in range(0, n_out_channels * 2, 2)], 
            dim=0)
        est_imag = torch.cat(
            [est[:, i :, :] for i in range(1, n_out_channels * 2, 2)], 
            dim=0)
        
        lbl_real = torch.cat(
            [lbl[:, i :, :] for i in range(0, n_out_channels * 2, 2)],
            dim=0)
        lbl_imag = torch.cat(
            [lbl[:, i :, :] for i in range(1, n_out_channels * 2, 2)], 
            dim=0)
        
        loss = (0.5 
                * torch.sum(torch.abs(est_real) 
